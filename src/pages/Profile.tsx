...
interface ProfileProps {
  user: UserType | null;
  onUserUpdate?: (user: UserType) => void; // Optional callback to sync parent state
}

export const Profile = ({ user: propUser, onUserUpdate }: ProfileProps) => {
  const toast = useToast();
  const [currentUser, setCurrentUser] = useState<UserType | null>(propUser);
  ...
  // Fetch latest user data from backend when prop changes
  useEffect(() => {
    if (!propUser) return;

    const fetchUser = async () => {
      try {
        const res = await fetch(`/api/users/${propUser.id}`);
        if (res.ok) {
          const freshUser = await res.json();
          setCurrentUser(freshUser);
          // Initialize editForm with fresh data
          setEditForm({
            name: freshUser.name || '',
            email: freshUser.email || '',
            phone: freshUser.phone || '',
            gender: freshUser.gender || '',
            vehicle_type: freshUser.vehicle_type || '',
            password: '',
  ...
  // Fetch ratings and stats when currentUser changes
  useEffect(() => {
    if (!currentUser) return;

    // Fetch ratings
    fetch(`/api/ratings/${currentUser.id}`)
      .then(res => res.json())
      .then(data => {
    ...
    // Fetch user stats
    Promise.all([
      fetch(`/api/rides/driver/${currentUser.id}`).then(r => r.json()),
      fetch(`/api/bookings/passenger/${currentUser.id}`).then(r => r.json())
    ]).then(([rides, bookings]) => {
      setStats({
  ...
  /**
   * FIX: Auto-save profile image to DB immediately on upload.
   *
   * Previously, handleImageUpdate only updated editForm local state.
   * If the user navigated away without clicking "Save", the uploaded
   * image was silently discarded. Now we persist it to the DB right
   * away, update localStorage, and notify the parent.
   */
  const handleImageUpdate = async (imageData: string) => {
    // Update local form state so the preview stays in sync
    setEditForm(prev => ({ ...prev, profile_image: imageData }));
    ...
    try {
      ...
      // Keep everything in sync: component state -> localStorage -> parent
      setCurrentUser(updatedUser);
      localStorage.setItem('agraride_user', JSON.stringify(updatedUser));
      if (onUserUpdate) onUserUpdate(updatedUser);

      toast.success(imageData ? 'Profile photo saved!' : 'Profile photo removed.');
    } catch (err) {
      console.error('Failed to auto-save profile image:', err);
      toast.error('Network error while saving photo. Please try again.');
    }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (normalize(editForm.name) !== normalize(currentUser.name)) {
        updateData.name = editForm.name.trim();
      }
      if (normalize(editForm.email) !== normalize(currentUser.email)) {
        updateData.email = editForm.email.trim();
      }
      if (normalize(editForm.phone) !== normalize(currentUser.phone)) {
        updateData.phone = editForm.phone.trim();
      }
      if (normalize(editForm.gender) !== normalize(currentUser.gender)) {
        updateData.gender = editForm.gender;
      }
      if (normalize(editForm.vehicle_type) !== normalize(currentUser.vehicle_type)) {
        updateData.vehicle_type = editForm.vehicle_type;
      }
      if (showPasswordField && editForm.password.trim()) {
        updateData.password = editForm.password.trim();
      }
      if (normalize(editForm.profile_image) !== normalize(currentUser.profile_image)) {
        updateData.profile_image = editForm.profile_image;
      }
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isEditing ? (
                      ...
                      <p className="mt-1 text-sm text-gray-900 truncate">{currentUser.email}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    {isEditing ? (
                      ...
                      <p className="mt-1 text-sm text-gray-900 capitalize">{currentUser.gender || 'Not specified'}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    {isEditing ? (
                      ...
                      <p className="mt-1 text-sm text-gray-900">{currentUser.vehicle_type || 'None'}</p>
                    )}
                  </div>
                </div>
